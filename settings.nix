...
}:
{
  type-with =
    ...
    let
      ...
      make-nullable-options =
        opts:
        lib.mapAttrs (
          name: value:
          if ((value._type or null) == "option") then
            value
            // (lib.optionalAttrs ((value.type.name or null) != "nullOr") {
              type = lib.types.nullOr value.type;
              default = null;
            })
          else if builtins.isAttrs value then
            make-nullable-options value
          else
            value
        ) opts;
      ...
      make-layout-options =
        ...
        in
        ordered-section (if make-nullable then (map make-nullable-options base) else base);
    in
    submodule (
      { options, ... }:
      {
        ...
        imports = make-ordered-options [
          ...
          {
            layout = make-layout-options { inherit options; };
          }
    ...
    if cfg == null then
      ...
      let
        ...
        override =
          f: old: new:
          let
            new' = lib.filterAttrsRecursive (_: v: v != null) new;
      ...
      normalize-nodes [
        ...
        (each' cfg.outputs (output: [
          (node "output" output.name [
            (toggle' "off" output [
              (nullable leaf "backdrop-color" output.backdrop-color)
              (nullable leaf "background-color" output.background-color)
              (nullable leaf "scale" output.scale)
              (flag' "focus-at-startup" output.focus-at-startup)
              (map' leaf transform "transform" output.transform)
              (nullable leaf "position" output.position)
              (nullable (map' leaf mode) "mode" output.mode)
        ...
        (plain "layout" (layout cfg.layout))
